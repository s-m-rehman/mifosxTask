...
public class SynchronousCommandProcessingService implements CommandProcessingService {
    ...
    private NewCommandSourceHandler findCommandHandler(final CommandWrapper wrapper) {
        ...
        } else if(wrapper.isClientEntityDetails()){
        	if(wrapper.isCreateClientEntityDetails()){
        		handler = this.applicationContext.getBean("createClientEntityDetailsCommandHandler",NewCommandSourceHandler.class);
        	}
        } else {
